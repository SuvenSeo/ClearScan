...
- **Biometric unlock:** `MainActivity.authenticateForVaultCrypto` passes a `BiometricPrompt.CryptoObject` from `VaultCrypto.createDecryptCipher()`. Successful authentication calls `markSessionAuthorized()`; `lockVault()` calls `clearSession()` and clears readable cache.
